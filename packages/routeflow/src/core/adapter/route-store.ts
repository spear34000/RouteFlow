...
export interface ListOptions<S extends SchemaDefinition> {
   ...
   *
   * **Performance note**: large `offset` values cause the database to scan and
   * discard rows. For tables with millions of rows, prefer cursor-based pagination
   * via `after` (keyset pagination on the primary key).
   *
   * @example
   * ```ts
   * // Page 3 of 20 results per page
   * await messages.list({ limit: 20, offset: 40 })
   * ```
   */
  offset?: number
  /**
   * Cursor-based pagination: return only rows whose `id` is strictly greater
   * than `after`.  Combine with `orderBy: 'id'` and `order: 'asc'` (defaults)
   * for efficient infinite-scroll / "load more" pagination that stays fast
   * even at millions of rows (uses the primary-key index).
   *
   * @example
   * ```ts
   * // Initial load
   * const page1 = await messages.list({ limit: 50, order: 'desc' })
   * // Next page: pass the smallest id from page1 as cursor
   * const page2 = await messages.list({ limit: 50, order: 'desc', after: page1.at(-1)?.id })
   * ```
   */
  after?: number
}
...
export class RouteTable<S extends SchemaDefinition> implements TableStore<InferRow<S>> {
   ...
   *
   * `db.prepare(sql)` parses and compiles a SQL query.  For the CRUD patterns
   * generated by RouteTable (SELECT, INSERT, UPDATE, DELETE), the SQL is
   * structurally identical across requests — only the bind parameters differ.
   * Caching the compiled statement eliminates redundant parse/compile work on
   * every call.
   *
   * SQLite's own internal statement cache is only 16 entries; for an app with
   * many tables this evicts frequently.  Our 64-entry per-table LRU keeps all
   * CRUD variants warm across the typical query mix.
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private readonly stmtCache = new Map<string, ReturnType<SQLiteStore['prepare']>>()
  private static readonly MAX_STMT_CACHE = 64
  ...
  /**
   * Return a cached compiled statement — compiling only on first encounter.
   * Re-inserts the entry on hit (Map insertion order = LRU order).
   * Evicts the least-recently-used entry when the cache is full.
   */
  private prepare(query: string): ReturnType<SQLiteStore['prepare']> {
    const cached = this.stmtCache.get(query)
    if (cached) {
      // Move to MRU end: delete then re-insert.
      this.stmtCache.delete(query)
      this.stmtCache.set(query, cached)
      return cached
    }
    const stmt = this.db.prepare(query)
    if (this.stmtCache.size >= RouteTable.MAX_STMT_CACHE) {
      // Evict LRU (first key in insertion order).
      const lru = this.stmtCache.keys().next().value!
      this.stmtCache.delete(lru)
    }
  ...
  async list(options: ListOptions<S> = {}): Promise<InferRow<S>[]> {
    ...
    if (after != null) {
      if (!Number.isInteger(after) || after < 0) {
        throw new Error(`Invalid after "${after}" — must be a non-negative integer`)
      }
    }
    ...
    // Cursor-based pagination: id > after (efficient keyset pagination)
    if (after != null) {
      whereParts.push('"id" > ?')
      params.push(after)
    }
   ...
   * @example
   * ```ts
   * const rows = await items.getMany([1, 2, 99])
   * // → [{ id: 1, ... }, { id: 2, ... }, null]
   * ```
   */
  async getMany(ids: number[]): Promise<(InferRow<S> | null)[]> {
    ...
    // Build id→row map for O(1) lookup; preserves input order and fills gaps with null.
    const rowMap = new Map(rows.map((r) => [(r as { id: number }).id, this.deserialize(r)]))
 ...
 *
 * Requires Node.js 22.13+ (built-in `node:sqlite`, no flag required).
 *
 * @example
 * ```ts
